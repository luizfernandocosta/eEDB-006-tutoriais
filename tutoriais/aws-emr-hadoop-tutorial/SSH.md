...
```bash
# Ver processos Java (NameNode, ResourceManager, etc)
jps
...
# Ver resultados do WordCount
hdfs dfs -ls /output/wordcount
hdfs dfs -cat /output/wordcount/part-r-00000 | sort -t$'\t' -k2 -nr | head -20
...
# Rodar WordCount manualmente (se quiser testar)
aws s3 cp s3://<ACCOUNT_ID>-emr-lab-wordcount/jars/wordcount.jar /tmp/wordcount.jar
hadoop jar /tmp/wordcount.jar WordCountApplication hdfs:///input/ hdfs:///output/test/
...
## 3. Tunel SSH para Web UI do Hadoop
...
## 5. Key pair vockey vs labsuser.pem
